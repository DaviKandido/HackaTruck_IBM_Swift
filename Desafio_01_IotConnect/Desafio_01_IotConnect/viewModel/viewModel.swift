...
class ViewModel: ObservableObject{
    ...
    @Published var umidade: [UmidadeStruct] = []
    
    func fetch(){
        guard let url = URL(string: "http://192.168.128.80:1880/umidadeGET") else {
            return
        }
        ...
        let task = URLSession.shared.dataTask(with: url){ [weak self] data, _, error in
            ...
            do{
                let parsed = try JSONDecoder().decode([UmidadeStruct].self, from: data)
...
//            
//            do {
//                let jsonData = try JSONEncoder().encode(umidadePost)
